...
class NetworkAIGUI:
    def __init__(self, root):
        ...
        self.ai_assistant = None  # 初始化为空，等待用户设置 API 密钥
    ...
    def create_widgets(self):
        ...
        # API 密钥输入框
        ttk.Label(right_frame, text="DASHSCOPE API 密钥").pack(pady=5)
        self.api_key_entry = tk.Entry(right_frame, show="*")
        self.api_key_entry.pack(fill=tk.X, padx=5, pady=5)

        ttk.Button(right_frame, text="保存 API 密钥", command=self.save_api_key).pack(pady=5)
    ...
    def save_api_key(self):
        ...
        if not api_key:
            messagebox.showerror("错误", "请输入有效的 DASHSCOPE API 密钥")
            return
        ...
        try:
            ...
            messagebox.showerror("错误", f"保存 API 密钥失败：{e}")
    ...
    def analyze_with_ai(self):
        if not self.ai_assistant:
            messagebox.showwarning("警告", "请先设置 DASHSCOPE API 密钥")
            return
    ...
    def _ai_analysis_thread(self, prompt):
        try:
            ...
            self.root.after(100, lambda: self.ai_output.insert(tk.END, f"AI分析失败：{e}\n"))
